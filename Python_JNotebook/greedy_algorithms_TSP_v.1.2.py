...
def show_route(route, info):
    ...
    if len(arr) > 1:
        for i in range(len(arr) - 1):
            ...
            ax.text(mid[0], mid[1], f'{time_seg:.0f}', color='black', fontsize=10,
                    ha='center', va='center', bbox=dict(facecolor='black', alpha=0.4, boxstyle='round,pad=0.2'))

        # Zusätzliche Grafik: Zeichne einen Pfeil vom Startpunkt zum ersten Ziel, der Richtungsbeginn der Route zeigt.
        arrow_start = arr[0]
        arrow_end = arr[1]
        arrow_vec = arrow_end - arrow_start
        # Der Pfeil zeigt vom Startpunkt (Depot) zum ersten Kunden in der Route.
        # Skaliert den Vektor auf eine angemessene Pfeillänge für bessere Sichtbarkeit.
        arrow_length = np.linalg.norm(arrow_vec)
        if arrow_length > 0:
            arrow_dir = arrow_vec / arrow_length * min(10, arrow_length)  # max. Länge 10 Einheiten
            ax.arrow(arrow_start[0], arrow_start[1], arrow_dir[0], arrow_dir[1],
                    head_width=3, head_length=5, fc='orange', ec='orange', linewidth=2)
